...
const int disMax=10; //ширина дорожки
const int color[4] = {1,2,3,5}; //цвета: красный, зелёный, жёлтый, сиреневый
char port[10] = {0};
...
void loop() {
  ...
  if (val=='5'){
    roll.write(180);
    int dis=distance.ping_cm();
    //Serial.println(dis);
    if ((dis<disMax)and(dis!=0)){
      roll.write(180);
      delay(200);
      roll.write(0);
      cnew=DeterColor(sensor.r, sensor.g, sensor.b);
